...
stop_on_comp_error="YES"
stop_on_comp_error="NO"
...
RunShyfem()
{
  ...
  if [ $status -ne 99 ]; then
    ...
    if [ $stop_on_comp_error = "YES" ]; then
      exit 1
    else
      return
    fi
...
HandleCompare()
{
  ...
  if [ $files1 != $files2 ]; then
    errortext="*** files are different... cannot compare $dir and $dir2"
    echo $errortext
    return
  fi
...
echo "chkonlydbg = $chkonlydbg"
  ...
  if [ -f $dir1/$file ]; then
    ...
    [ $error -ne 0 ] && echo "*** error comparing between $oldnp - $np" >> $LOG
  else
  ...
  if [ $errortot -ne 0 ]; then
    echo "    *** a total of $errortot errors found"
    (( errorall += errortot ))
  fi
}
...
if [ $compare = "YES" ]; then
  action="compare"
  CompareSims
  [ $errorall -ne 0 ] && CompareSims	# try one more time
